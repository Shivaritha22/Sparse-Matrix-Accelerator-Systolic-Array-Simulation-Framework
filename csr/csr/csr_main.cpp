...
int main() {
    ...
    // ---- prepare big decompressed output ----
    ...
    for (int tr = 0; tr < TILES; ++tr) {
        for (int tc = 0; tc < TILES; ++tc) {
            ...
            // clear CSR outputs for this tile
            for (int i = 0; i < MAX_ROWS; ++i) offset[i] = 0;
            for (int i = 0; i < MAX_NNZ; ++i) col[i] = 0;
            for (int i = 0; i < MAX_NNZ; ++i) val[i] = 0.0f;
            size = 0;

            csr_format(A_tile, TILE, TILE, offset, col, val, &size);

            if (size != offset[TILE - 1]) {
                cerr << "CSR invariant failed on tile (" << tr << "," << tc
                     << "): size=" << size << " last_offset=" << offset[TILE - 1] << endl;
            }

            
            total_meta_elems = total_meta_elems + size + TILE;
            ...
            fout << "\n";
            fout << "Non-zero values\n";
            for (int i = 0; i < size; ++i) fout << val[i] << " ";
            fout << "\n";
            fout << "Row offsets\n";
            for (int i = 0; i < TILE; ++i) fout << offset[i] << " ";
            fout << "\n\n";
            ...
            
            {
                ...
                for (int i = 0; i < size; ++i) fcomp << col[i] << " ";
                fcomp << "\n";
                fcomp << "Non-zero values\n";
                for (int i = 0; i < size; ++i) fcomp << val[i] << " ";
                fcomp << "\n";
                fcomp << "Row offsets\n";
                for (int i = 0; i < TILE; ++i) fcomp << offset[i] << " ";
                fcomp << "\n";
                fcomp.close();
            }
            ...
            csr_decompression(A_tile_decomp, TILE, TILE, offset, col, val, &size);

            
            int cycles_tile = TILE + TILE + size - 1; // 64 + 64 + nnz - 1
    ...
    // functionality check
    {
        ...
        if (identical) {
            cout << "CSR Compression and Decompression: Successful" << endl;
        }
    }
    ...
    
    {
        ...
        fsum << "\nTotal CSR decompression cycles (all tiles): " << total_cycles_all_tiles;
        fsum << "\nAverage CSR decompression cycles per tile: " << avg_cycles_per_tile << endl;
